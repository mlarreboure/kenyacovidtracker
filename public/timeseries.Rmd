...
Price index based on 2018 USD, using KLPS household consumption weights. 
...
<font size="5">**Total Weekly Consumption**</font>  
...
fig <- fig %>% layout(
    title = "Total Weekly Consumption - USD, PPP"
) %>% animation_opts(frame = 2000, transition = 500) %>%
  partial_bundle() %>% toWebGL()
fig
```
...
fig <- fig %>% layout(
    title = "Percentage of respondents who reported using a face mask"
) %>% animation_opts(frame = 2000, transition = 500)%>%
  partial_bundle() %>% toWebGL()
fig
```
