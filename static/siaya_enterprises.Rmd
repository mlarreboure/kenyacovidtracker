...
# Add retrospective averages from the first 4 weeks of the survey
rev_prof_s <- data.frame("week" = c(-11,-10,-9,-7,-6,-3),
                  "rev_wins_wins_wins_rt" = c(NA,223.6966,NA,162.1954,NA,138.2139),
...
rev_prof_s$rev_wins_wins_wins_rt <- rev_prof_s$rev_wins_wins_wins_rt*100/223.6966
rev_prof_s$rev_wins_wins_wins_mf <- rev_prof_s$rev_wins_wins_wins_mf*100/208.973
rev_prof_s$rev_wins_wins_wins <- rev_prof_s$rev_wins_wins_wins*100/137.3485
...
tmp <- rbind(tmp, rev_prof_s[c(17,18),])
rev_prof_s <- tmp
...
``` {r invest}
  ...
  dyAxis("x", pixelsPerLabel = 90 , drawGrid = FALSE, label = "Survey Week") %>% #grid
...
layoffs <- data.frame("Month" = c("January", "February", "March", "April", "May"),
                  "Frequency" = c((sum(ent_outcomes$s3_q8_laidoff_jan, na.rm = TRUE)+42)*100/total,
                                  (sum(ent_outcomes$s3_q8_laidoff_feb, na.rm = TRUE)+50)*100/total,
                                  (sum(ent_outcomes$s3_q8_laidoff_mar, na.rm = TRUE)+197)*100/total,
                                  (sum(ent_outcomes$s3_q8_laidoff_apr, na.rm = TRUE)+8+ 107)*100/total,
                                  (sum(ent_outcomes$s3_q8_laidoff_may, na.rm = TRUE)+8 +98)*100/total))
layoffs$Month <- factor(layoffs$Month, levels = layoffs$Month)
ggplot(layoffs, aes(Month, Frequency)) + geom_col(fill="dodgerblue3", colour="dodgerblue3") + ylab("% Layoffs") + theme_classic()
```


*** 

<font size="5">**Temporal distribution of business closures relative to February's operating level**</font>

```{r}
closed <- data.frame("Month" = c("January", "February", "March", "April", "May"),
                  "Proportion" = c((62/2797)*100, (46/2797)*100, (236/2797)*100, (169/2797)*100, ((45+39)/2797)*100))
closed$Month <- factor(closed$Month, levels = closed$Month)

ggplot(closed, aes(Month, Proportion)) + geom_col(fill="dodgerblue3", colour="dodgerblue3") + ylab("% Business closures")+ theme_classic()
```
