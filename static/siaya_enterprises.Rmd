...
tmp <- data.frame("week" = c(-10),
                  "investment_wins" = c(i_feb)) #2.291782 before
inv_r <- rbind(tmp,inv)
inv_r <- rbind(c(-11,NA), inv_r)
tmp <- data.frame("week" = c(-10),
                  "wages_wins" = c(w_feb)) #28.96816
wages_r <- rbind(tmp,wages)
wages_r <- rbind(c(-11,NA), wages_r)
```
...
*** 
<font size="3">**Revenue by sector in the past 14 days (2 week average)**</font>
```{r}
