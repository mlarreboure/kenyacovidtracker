...
tmp[tmp$week == 5.5,"i1_wins"] = rev_prof_s[18,"i1_wins"]
tmp <- rbind(tmp, c(7.5, NA,NA,NA,(rev_prof_s[19,"i1_wins"]+rev_prof_s[20,"i1_wins"])/2)) # Setting week 7 to missing values
tmp <- rbind(tmp, rev_prof_s[21:22,])
rev_prof_s <- tmp
...
inv <- rbind(inv, c(7, NA)) ## Exclude week 7 since very few data points
inv_r <- rbind(tmp,inv)
inv_r <- rbind(c(-2,NA), inv_r)
inv_r <- rbind(inv_r, c(10, NA))
tmp <- data.frame("week" = c(-1),
...
``` {r loan}
loan <- loan[loan$week < 9,] # Data missing after this week for these variables
loan <- rbind(loan, c(8, NA, NA, NA))
